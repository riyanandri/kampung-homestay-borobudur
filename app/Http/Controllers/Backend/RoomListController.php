...
class RoomListController extends Controller
{
    ...
    {
        $room_number_list = RoomNumber::with(['roomType','lastBooking.booking:id,check_in,check_out,status,code,name,phone'])->orderBy('room_type_id','asc')
        ...
        return view('backend.room.room_list.view_room_list',compact('room_number_list'));
    }
    ...
    public function addRoomList()
    {
        ...
        return view('backend.room.room_list.add_room_list',compact('roomtype'));
    }
    ...
    public function storeRoomList(Request $request)
    {
        if ($request->check_in == $request->check_out) {
            $request->flash();
            $notification = array(
                'message' => 'Anda memasukkan tanggal yang sama',
                'alert-type' => 'error'
            );
        ...
        if ($request->available_room < $request->number_of_rooms) {
            $request->flash();
            $notification = array(
                'message' => 'Anda memasukkan jumlah kamar maksimum',
                'alert-type' => 'error'
            );
        ...
        if ($room->room_capacity < $request->number_of_person ) {
            $notification = array(
                'message' => 'Anda memasukkan jumlah orang yang melebihi kapasitas kamar',
                'alert-type' => 'error'
            );
